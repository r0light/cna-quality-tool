...
const measures = {
    ...
    "averageSystemCoupling": {
        ...
        "calculationFormula": "\\frac{ \\displaystyle\\sum_{i=1}^{|L|} \\begin{cases} 0.1 &\\text{if } l.targetEndpoint.kind = \\text{\"send event\"} \\\\ 0.5 &\\text{if } l.targetEndpoint.kind = \\text{\"command\"} \\\\ 0.2 &\\text{else } \\end{cases} + \\frac{\\displaystyle\\sum_{i=1}^{|l.targetEndpoint.RDA|} \\begin{cases} 0.1 &\\text{if } l.targetEndpoint.RDA.usage\\_relation = \\text{\"persistence\"} \\\\ 0.5 &\\text{if } l.targetEndpoint.RDA.usage\\_relation = \"cached-usage\" \\\\ 0.25 &\\text{else } \\end{cases}   }{|l.targetEndpoint.RDA|} } { |C| }",
        "helperFunctions": [],
    ...
    "ratioOfRequestTracesThroughGateway": {
        ...
        "calculationFormula": "\\frac{|\\Set{ rt | rt \\in RT \\land ((rt.referencedEndpoint \\in c.providedEndpoints \\land c.externalIngressProxiedBy = p \\land p \\in PBS \\land p.kind = \\text{\"API Gateway\"}) \\lor (rt.referencedEndpoint \\in g.providedEndpoints \\land g \\in PBS \\land g.kind = \\text{\"API Gateway\"})) }|}{|RT|}",
        "helperFunctions": [],
        "sources": ["Zdun2023a"],
        "applicableEntities": [ENTITIES.SYSTEM],
    },
